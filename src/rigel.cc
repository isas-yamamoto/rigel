...
void Rigel::Init(const char* dirname,
  ...
  this->max_file_size_ = max_file_count * block_size;
...
bool Rigel::Open(const int index,
                    sli::stdstreamio& data_io,
                    sli::stdstreamio& index_io,
                    const char* mode,
                    const char* err_mode) {
  ...
  if (OpenData(data_io,file_index,mode,err_mode)) {
    data_io.seek(file_offset, SEEK_SET);
  } else {
    return false;
  }
  
  if (OpenIndex(index_io,mode,err_mode)) {
    index_io.seek(index,SEEK_SET);
  } else {
    return false;
  }
 ...
 *  失敗したときはfalseを返す。
 */
bool Rigel::OpenData(sli::stdstreamio& data_io,
                     int file_index,
                     const char* mode,
                     const char* err_mode) {
  sli::tstring filename;
  filename.printf("%s/%s.%04d", this->dirname_, this->key_, file_index);
  if (data_io.open(mode, filename.cstr()) < 0) {
    if (data_io.open(err_mode, filename.cstr())<0) {
      sli::stdstreamio sio;
      sio.printf("ERROR Rigel::Open filename=%s\n", filename.cstr());
      return false;
    }
  }
  return true;
 ...
 *  失敗したときはfalseを返す。
 */
bool Rigel::OpenIndex(sli::stdstreamio& index_io,
                      const char* mode,
                      const char* err_mode) {
  sli::tstring filename;
  filename.printf("%s/%s.index", this->dirname_, this->key_);
  if (index_io.open(mode, filename.cstr()) < 0) {
    if (index_io.open(err_mode, filename.cstr())<0) {
      return false;
    }
  }
  return true;
...
ssize_t Rigel::Write(const int index,
                     ...
                     size_t size) {
  sli::stdstreamio data_io, index_io;
  if(this->Open(index, data_io, index_io, "r+b", "w+b")) {
    ssize_t ret = data_io.write(data, size);
    char c = 1;
    index_io.write(&c,1);
    return ret;
  }
...
bool Rigel::ScanInit(const int start) {
  if (this->OpenIndex(this->scan_io,"rb","rb")) {
    this->scan_io.seek(0, SEEK_SET);
    this->scan_index_ = 0;
    this->scan_offset_ = 0;
    this->scan_size_=this->scan_io.read(this->scan_buf_,BUF_SIZE);
    return true;
  }
  return false;
}
